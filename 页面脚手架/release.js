...
const schoolLoading_bak  = db.collection('schoolLoading')
const jump = db.collection('jumpPage_bak')
Page({
  ...
  onLoad: async function (options) {
    ...
    // 注意！这个只能拉20个学校，我也希望未来我们能超过100个
    console.log(await schoolLoading_bak.count())
    let total = (await schoolLoading_bak.count()).total
    let max_num = 20
    var res = []
    for(var i=0;i<total;i++){
      if(i*max_num<total){
        var res1 = (await schoolLoading_bak.where({}).skip(i * max_num).limit(max_num).field({ //显示哪些字段
  ...
  chooseTemperature: async function (e) {
    ...
    // let total =(await jump.where({schoolName : schoolName}).count()).total
    // let max_num = 20
    // var res = []
    // for(var i=0;i<total;i++){
    //   if(i*max_num<total){
    //     var res1 = (await jump.where({schoolName : schoolName}).skip(i * max_num).limit(max_num).get()).data
    //     console.log(res1,"222223")
    //     res.push(res1)
    //   }
    // }
    var jumpData = await (jump.where({schoolName : schoolName}).get()).data
    // var jumpData = res.reduce((acc, cur) =>{
    //   return acc.concat(cur)
    // })
  ...
  input_name(e){
    console.log(e)
    this.setData({name_:e.detail.value})
  },
  ...
  submit:function(e){
    ...
    let peizhi = {
      ...
      youxianji:this.data.youxianji,
      ishead:this.data.ishead || "有"
    }
    console.log(peizhi)
    if(!this.data.name_){
      wx.showToast({
    ...
    if(!this.data.tem){
      ...
      wx.cloud.callFunction({
        ...
        data:{
          ...
          school:this.data.tem,
          isTj:this.data.everInDangerRegion,
          code:this.data.text,
          peizhi
        },
        success(res){
  ...
  youxianji_input(e){
    this.setData({youxianji:e.detail.value})
  },
  ishead_input(e){
    this.setData({ishead:e.detail.value})
  },
  dizhi_input(e){
  ...
  <view class="container">
    ...
    <view class="write_Info_Container">
      ...
      <view class="temperature" >
        ...
        <view style="width:80%">
          <input style="width:60%" type="text" placeholder="输入名称" bindinput="everIndangerPlaceText" bindinput="input_name"/>
        </view>
      </view>
      ...
      <view class="temperature" wx:if="{{everInDangerRegion === '是'}}">
        ...
        <view style="width:80%">
          <input style="width:60%" type="text" placeholder="详细地址" bindinput="dizhi_input" value="{{everIndangerRegionText}}" />
        </view>
      </view>
      ...
      <view class="temperature" wx:if="{{everInDangerRegion === '是'}}">
        <text style="line-height:64rpx">优先级</text>
        <view style="width:80%">
          <input style="width:60%" type="text" placeholder="输入优先级" bindinput="youxianji_input" value="{{everIndangerRegionText}}" />
        </view>
      </view>
      <view class="temperature" wx:if="{{everInDangerRegion === '是'}}">
      <text style="line-height:64rpx">是否有头</text>
      <view style="width:80%">
        <input style="width:60%" type="text" placeholder="输入有或无(默认有，不懂找我！！！)" bindinput="ishead_input" value="{{everIndangerRegionText}}" />
      </view>
    </view>
      <view wx:else></view>
